define(function(require, exports, module) {
    ...
    var FreeRentStageItemModel = Backbone.Model.extend({
        initialize: function (attr, options) {
            this.sourceModel = options.sourceModel || {};
        },
        getContractDeadLine: function () {
        ...
        validateStageDate: function () {
            ...
            var maturityTimeValid = deadLine.maturityTime && new Date(deadLine.maturityTime) < new Date(result.endDate);
            if(effectiveTimeValid || maturityTimeValid) {
                this.trigger('freerentperiod:validate', false, '免租期阶段应在合同期限之内');
                return false;
            }else {
                this.trigger('freerentperiod:validate', true);
                return true;
            }
        }
    });

    var RentalContractModel = Backbone.Model.extend({
        urlRoot: './',
    ...
    exports.RentalContractModel = RentalContractModel;
    exports.PayStageItemModel = PayStageItemModel;
    exports.ExtraRuleItemModel = ExtraRuleItemModel;
    exports.FreeRentStageItemModel = FreeRentStageItemModel;
