...
$.fn.show_tooltip = function(text, ico, backgroundColor, borderColor, fontColor) {
    $(this).tooltip('destroy');
    var rid = 'tlt_' + parseInt(new Date().getTime());
    var marker = '<div id="'+rid+'"</div>';
    var icon = '<i class="glyphicon glyphicon-' + ico + '"></i>&nbsp;&nbsp;';
    $(this).tooltip(
      {title: icon+text+marker, html: true, trigger: 'manual', placement: 'bottom'});
    $(this).tooltip('show');
    $('#'+rid).parent().css({'background-color': backgroundColor, 'padding-top': '10px', 'max-width': '500px', 'padding-bottom': '10px', 'padding-left': '12px', 'padding-right': '12px', 'color': fontColor});
    $('#'+rid).parent().prev().css({'border-bottom-color': borderColor});
}
  
$.fn.show_ok_tooltip = function(text) {
    $(this).show_tooltip(text, 'info-sign', '#DFF0D8', '#D6E9C6', '#468847');
}

$.fn.show_err_tooltip = function(text) {
    $(this).show_tooltip(text, 'remove', '#F2DEDE', '#EBCCD1', '#B94A48');
}
...
$.fn.bt_validate = function() {
  ...
  $.bt_validate.form.find('input[data-validate],select[data-validate],textarea[data-validate]').blur(function() {
    ...
    var validate_params = $(this).data('validate').split('|');
    ...
    for(var i in validate_params) {
      ...
      if(typeof(res) != 'string') {
          if(!res) {
            ...
            if($(this).data('validate-' + fn_name + '-message')) {
            	tl_text = $(this).data('validate-' + fn_name + '-message')
            }
...
$.bt_validate.ajax_check = function(params) {
  ...
  this.fn_validate = function(value) {
    ...
    $.bt_validate.block()
    this.show_tooltip(ajax_check.params.msg_checking, 'refresh', '#D9EDF7', '#BCE8F1', '#3A87AD');
