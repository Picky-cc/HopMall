...
public class CommHeader {
	...
	public CommHeader iniForQueryCashFlow(Map<String, String> config, String bodyLength, Date currentTime) {
		commonInitialize(config, bodyLength, currentTime);
		this.TRADING_CODE = String.format("%-6s", BankCorpEps.PAB_QUERYCASHFLOW_CODE);
		return this;
	}
	
	public CommHeader iniForCredit(Map<String, String> config, String bodyLength, Date currentTime) {
		commonInitialize(config, bodyLength, currentTime);
		this.TRADING_CODE = String.format("%-6s", BankCorpEps.PAB_CREDIT_CODE);
		return this;
	}
	
	public CommHeader ini_for_retrieve_balance(Map<String, String> config, String body_length, Date currentTime) {
		commonInitialize(config, body_length, currentTime);
		this.TRADING_CODE = String.format("%-6s", config.getOrDefault("RETRIEVE_BALANCE_CODE", "4001").toString());
		return this;
	}
	
	public CommHeader iniForQueryCredit(Map<String, String> config, String bodyLength, Date currentTime) {
		commonInitialize(config, bodyLength, currentTime);
		this.TRADING_CODE = String.format("%-6s", BankCorpEps.PAB_QUERYCREDIT_CODE);
		return this;
	}
	
	private void commonInitialize(Map<String, String> config, String body_length, Date currentTime) {
		...
		this.ENTERPRISE_CODE = config.getOrDefault("enterpriseCode", StringUtils.EMPTY).toString();
		...
		this.SERIAL_NO = String.format("%-20s", DateUtils.format(currentTime, "yyyyMMddHHmmssSSS"));//TODO
